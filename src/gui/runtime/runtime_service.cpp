...
bool canResolveSubsystemKey(
    ...
    if (variableIds.find(key) != variableIds.end()) {
        return true;
    }

    const auto aliasIt = aliases.find(key);
    if (aliasIt != aliases.end() && variableIds.find(aliasIt->second) != variableIds.end()) {
        return true;
    }
...
bool RuntimeService::renameCheckpoint(const std::string& fromLabel, const std::string& toLabel, std::string& message) {
    ...
    try {
        ...
        if (checkpoints_.find(toLabel) != checkpoints_.end()) {
