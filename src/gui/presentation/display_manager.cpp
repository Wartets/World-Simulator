...
const std::unordered_map<std::string, std::vector<std::string>>& emptyDisplayFieldTags() {
    static const std::unordered_map<std::string, std::vector<std::string>> empty;
    return empty;
}

DisplayManagerParams DisplayManagerParams::sanitized() const {
    DisplayManagerParams tuned = *this;
    tuned.autoWaterQuantile = std::clamp(tuned.autoWaterQuantile, 0.0f, 1.0f);
    tuned.waterLevel = std::clamp(tuned.waterLevel, 0.0f, 1.0f);
    tuned.lowlandThreshold = std::clamp(tuned.lowlandThreshold, 0.0f, 1.0f);
    tuned.highlandThreshold = std::clamp(tuned.highlandThreshold, tuned.lowlandThreshold + 0.01f, 1.0f);
    tuned.waterPresenceThreshold = std::clamp(tuned.waterPresenceThreshold, 0.0f, 1.0f);
    tuned.shallowWaterDepth = std::clamp(tuned.shallowWaterDepth, 0.0f, 0.5f);
...
bool DisplayManagerParams::validate(std::string& errorMessage) const {
    const auto finite = [](const float v) { return std::isfinite(v); };
    if (!finite(waterLevel) || !finite(autoWaterQuantile) || !finite(lowlandThreshold) ||
        !finite(highlandThreshold) || !finite(waterPresenceThreshold) ||
        !finite(shallowWaterDepth) || !finite(highMoistureThreshold)) {
        errorMessage = "display_params_invalid reason=non_finite";
        return false;
    }
    errorMessage.clear();
    return true;
...
DisplaySnapshotRequest DisplaySnapshotRequest::sanitized() const {
    DisplaySnapshotRequest out = *this;
    if (!snapshot.fields.empty()) {
        out.primaryFieldIndex = std::clamp(primaryFieldIndex, 0, static_cast<int>(snapshot.fields.size()) - 1);
    } else {
        out.primaryFieldIndex = 0;
    }
    return out;
}

bool DisplaySnapshotRequest::validate(std::string& errorMessage) const {
    if (snapshot.fields.empty()) {
        errorMessage = "display_snapshot_request_invalid reason=no_fields";
        return false;
    }
    if (primaryFieldIndex < 0 || primaryFieldIndex >= static_cast<int>(snapshot.fields.size())) {
        errorMessage = "display_snapshot_request_invalid reason=primary_field_index_out_of_range";
        return false;
    }
    errorMessage.clear();
    return true;
...
DisplayTerrainPreviewRequest DisplayTerrainPreviewRequest::sanitized() const {
    DisplayTerrainPreviewRequest out = *this;
    if (out.label.empty()) {
        out.label = "preview";
    }
    return out;
...
bool DisplayTerrainPreviewRequest::validate(std::string& errorMessage) const {
    if (terrain.get().empty() && water.get().empty()) {
        errorMessage = "display_terrain_request_invalid reason=no_inputs";
        return false;
    }
    errorMessage.clear();
    return true;
...
DisplayPreviewComponentsRequest DisplayPreviewComponentsRequest::sanitized() const {
    DisplayPreviewComponentsRequest out = *this;
    if (out.label.empty()) {
        out.label = "preview";
    }
    return out;
...
bool DisplayPreviewComponentsRequest::validate(std::string& errorMessage) const {
    if (primary.get().empty() && terrain.get().empty() && water.get().empty() &&
        humidity.get().empty() && windU.get().empty() && windV.get().empty()) {
        errorMessage = "display_preview_components_request_invalid reason=no_inputs";
        return false;
    }
    errorMessage.clear();
    return true;
...
DisplayBuffer buildDisplayBufferFromSnapshot(
    ...
    const DisplayManagerParams& params) {
    DisplayBuffer result;
    std::string requestValidationMessage;
    if (!request.validate(requestValidationMessage)) {
        result.label = "Invalid snapshot display request";
        return result;
    }

    std::string paramsValidationMessage;
    if (!params.validate(paramsValidationMessage)) {
        result.label = "Invalid display params";
        return result;
    }

    const DisplaySnapshotRequest tunedRequest = request.sanitized();
    const DisplayManagerParams tunedParams = params.sanitized();

    const auto& snapshot = tunedRequest.snapshot;
    if (snapshot.fields.empty()) {
    ...
    const int clampedPrimary = tunedRequest.primaryFieldIndex;

    const auto primary = mergedFieldValues(fields[static_cast<std::size_t>(clampedPrimary)], tunedRequest.includeSparseOverlay);

    const auto& fieldDisplayTags = tunedRequest.fieldDisplayTags.get();
    ...
    const std::vector<float> terrain = (terrainIdx >= 0)
        ? mergedFieldValues(fields[static_cast<std::size_t>(terrainIdx)], tunedRequest.includeSparseOverlay)
        : primary;
    const std::vector<float> water = (waterIdx >= 0)
        ? mergedFieldValues(fields[static_cast<std::size_t>(waterIdx)], tunedRequest.includeSparseOverlay)
        : std::vector<float>(primary.size(), 0.0f);
    const std::vector<float> humidity = (humidityIdx >= 0)
        ? mergedFieldValues(fields[static_cast<std::size_t>(humidityIdx)], tunedRequest.includeSparseOverlay)
        : std::vector<float>(primary.size(), 0.0f);
    const std::vector<float> windU = (windUIdx >= 0)
        ? mergedFieldValues(fields[static_cast<std::size_t>(windUIdx)], tunedRequest.includeSparseOverlay)
        : std::vector<float>(primary.size(), 0.0f);
    const std::vector<float> windV = (windVIdx >= 0)
        ? mergedFieldValues(fields[static_cast<std::size_t>(windVIdx)], tunedRequest.includeSparseOverlay)
        : std::vector<float>(primary.size(), 0.0f);
    ...
    return buildDisplayBufferCore(
        ...
        windU,
        windV,
        tunedRequest.displayType,
        tunedParams,
        fields[static_cast<std::size_t>(clampedPrimary)].spec.name);
}
...
DisplayBuffer buildDisplayBufferFromSnapshot(
    ...
    const DisplayManagerParams& params,
    const std::unordered_map<std::string, std::vector<std::string>>& fieldDisplayTags) {
    return buildDisplayBufferFromSnapshot(
        DisplaySnapshotRequest{snapshot, primaryFieldIndex, displayType, includeSparseOverlay, std::cref(fieldDisplayTags)},
        params);
}
...
DisplayBuffer buildDisplayBufferFromTerrain(
    ...
    std::string requestValidationMessage;
    if (!request.validate(requestValidationMessage)) {
        result.label = "Invalid terrain preview request";
        return result;
    }
    ...
    const DisplayTerrainPreviewRequest tunedRequest = request.sanitized();
    const DisplayManagerParams tunedParams = params.sanitized();

    const std::vector<float> safeTerrain = tunedRequest.terrain.get().empty()
        ? std::vector<float>(64u, 0.5f)
        : tunedRequest.terrain.get();
    std::vector<float> safeWater;
    if (tunedRequest.water.get().empty()) {
        safeWater.assign(safeTerrain.size(), 0.0f);
    } else {
        safeWater = tunedRequest.water.get();
    ...
    DisplayBuffer buildDisplayBufferFromTerrain(
        ...
        const DisplayManagerParams& params,
        const char* label) {
        return buildDisplayBufferFromTerrain(
        DisplayTerrainPreviewRequest{std::cref(terrain), std::cref(water), displayType, label == nullptr ? "preview" : label},
        params);
}
...
DisplayBuffer buildDisplayBufferFromPreviewComponents(
    ...
    std::string requestValidationMessage;
    if (!request.validate(requestValidationMessage)) {
        result.label = "Invalid preview components request";
        return result;
    }
    ...
    const DisplayPreviewComponentsRequest tunedRequest = request.sanitized();
    const DisplayManagerParams tunedParams = params.sanitized();

    const std::size_t minSize = std::max<std::size_t>(1u, tunedRequest.primary.get().size());

    std::vector<float> safePrimary = tunedRequest.primary.get();
    ...
    std::vector<float> safeTerrain = fitToPrimarySize(tunedRequest.terrain.get(), 0.0f);
    std::vector<float> safeWater = fitToPrimarySize(tunedRequest.water.get(), 0.0f);
    std::vector<float> safeHumidity = fitToPrimarySize(tunedRequest.humidity.get(), 0.0f);
    std::vector<float> safeWindU = fitToPrimarySize(tunedRequest.windU.get(), 0.0f);
    std::vector<float> safeWindV = fitToPrimarySize(tunedRequest.windV.get(), 0.0f);
...
DisplayBuffer buildDisplayBufferFromPreviewComponents(
    ...
    const DisplayManagerParams& params,
    const char* label) {
    return buildDisplayBufferFromPreviewComponents(
        DisplayPreviewComponentsRequest{
            std::cref(primary),
            std::cref(terrain),
            std::cref(water),
            std::cref(humidity),
